...
def training_outputs(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    ...
    if save_vids:
        ...
        if epoch % 500 == 0:
            ...
            if epoch % 1000 == 0:
                out = diffusion.forward_backward(ema, x, "whole", args['sample_distance'] // 2, denoise_fn="noise_fn")
                #@imgs = [[ax.imshow(1 - gridify_output(x, row_size), cmap='gray', animated=True)] for x in out]
            else:
                out = diffusion.forward_backward(ema, x, "whole", args['sample_distance'] // 4, denoise_fn="noise_fn")
                #imgs = [[ax.imshow(1 - gridify_output(x, row_size), cmap='gray', animated=True)] for x in out]
